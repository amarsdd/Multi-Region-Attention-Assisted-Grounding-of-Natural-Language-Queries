...
class Dataset(object):
    ...
    def __init__(self, class_map=None, base_image_path=''):
    ...
    def load_image(self, image_id):
        ...
        # Load image
        # print(image_id)
        new_path =  self.base_image_path  + self.image_info[image_id]['path']
        image = skimage.io.imread(new_path)
...
def resize_bbox(bbox, scale, padding, crop=None):
    ...
    bbox[:, [0, 2]] = bbox[:, [0, 2]] + padding[0][1]
    bbox[:, [1, 3]] = bbox[:, [1, 3]] + padding[1][0]
...
def compute_ap(gt_boxes, gt_class_ids, pred_boxes, pred_class_ids, pred_scores, iou_threshold=0.5):
    ...
    return mAP, precisions, recalls, overlaps, (pred_match > -1).astype(np.float32), (gt_match > -1).astype(np.float32)
    ...
    for iou_threshold in iou_thresholds:
        ap, precisions, recalls, overlaps, _, _ =\
            compute_ap(gt_box, gt_class_id, gt_mask,
                        pred_box, pred_class_id, pred_score, iou_threshold=iou_threshold)
        if verbose:
